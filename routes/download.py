...
def download_file(job_id, filename):
    ...
    try:
        ...
        if not job:
            return jsonify({"error": "Lost artifact", "message": "Job not found"}), 404

        if job.status != "completed":
            return (
                jsonify({"error": "Artifact not ready", "message": f"Job status: {job.status}"}),
                400,
            )
        ...
        if filename not in output_files:
            return (
                jsonify({"error": "Artifact not found", "message": "File not in job outputs"}),
                404,
            )
        ...
        if not os.path.exists(filepath):
            ...
            return (
                jsonify({"error": "Artifact vanished", "message": "File not found on server"}),
                404,
            )

        # Log download
        AuditLog.log_action("file_download", details={"job_id": job_id, "filename": filename})
        ...
        return jsonify({"error": "Retrieval failed", "message": "Could not download file"}), 500
...
def download_all(job_id):
    ...
    try:
        ...
        if job.status != "completed":
            return (
                jsonify({"error": "Artifacts not ready", "message": f"Job status: {job.status}"}),
                400,
            )

        output_files = job.get_output_files()
        if not output_files:
            return jsonify({"error": "No artifacts", "message": "No output files available"}), 404
        ...
        # Log download
        AuditLog.log_action(
            "batch_download", details={"job_id": job_id, "file_count": len(output_files)}
        )
        ...
        return jsonify({"error": "Collection failed", "message": "Could not create archive"}), 500
