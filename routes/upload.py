...
ALLOWED_EXTENSIONS = set(os.getenv('ALLOWED_EXTENSIONS', 'mp4,mov,avi,mkv,mp3,wav,flac,m4a').split(','))
...
def upload_file():
    ...
    try:
        ...
        # Log the upload
        AuditLog.log_action('file_upload', details={
            'job_id': job_id,
            'filename': original_filename,
            'size': file_info['size']
        })
...
def validate_upload():
    ...
    try:
        ...
        return jsonify({
            'valid': True,
            'message': 'The artifact is worthy of the forge'
        }), 200
