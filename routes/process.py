...
def get_job_status(job_id):
    ...
    try:
        ...
        return (
            jsonify({"error": "Vision obscured", "message": "Could not retrieve job status"}),
            500,
        )
...
def cancel_job(job_id):
    ...
    try:
        ...
        if job.status == "completed":
            return (
                jsonify(
                    {"error": "Artifact already forged", "message": "Cannot cancel completed job"}
                ),
                400,
            )

        if job.status == "failed":
            return jsonify({"error": "Forge already cooled", "message": "Job already failed"}), 400
        ...
        return (
            jsonify({"success": True, "message": "The forging has been halted", "job_id": job_id}),
            200,
        )

    except Exception as e:
        logger.error(f"Cancel error: {str(e)}", exc_info=True)
        return jsonify({"error": "Cannot extinguish", "message": "Could not cancel job"}), 500
...
def list_jobs():
    ...
    try:
        ...
        return (
            jsonify({"success": True, "jobs": [job.to_dict() for job in jobs], "count": len(jobs)}),
            200,
        )

    except Exception as e:
        logger.error(f"List jobs error: {str(e)}", exc_info=True)
        return jsonify({"error": "Chronicles lost", "message": "Could not retrieve jobs"}), 500
