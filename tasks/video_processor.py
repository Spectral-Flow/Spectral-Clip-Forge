                ...
                try:
                    if has_video:
                        ...
                        success = process_video_clip(
                            input_path, output_path, spec, options
                        )
                    else:
                        # Process audio - create visualizer video
                        success = process_audio_to_video(
                            input_path, output_path, spec, options
                        )
    ...
    try:
        ...
        if options.get('watermark', True):
            ...
            if os.path.exists(watermark_path):
                ...
                filters.append(
                    f"[0:v]overlay=W-w-10:H-h-10:format=auto,format=yuv420p[v]"
                )
        ...
        if options.get('effects') == 'spectral':
            ...
            filters.append(
                "curves=r='0/0 0.5/0.58 1/1':g='0/0 0.5/0.48 1/1':b='0/0.1 0.5/0.7 1/1'"
            )
